...
export default function FullName() {
  ...
  const validateFullName = (text: string) => {
    ...
    if (!trimmedValue) {
      setErrors({ error: "Full name is required." });
      setIsDisabled(true);
    } else if (trimmedValue.length < 2) {
      setErrors({ error: "Name must be at least 2 characters." });
      setIsDisabled(true);
    } else {
      setErrors({});
      setIsDisabled(false);
  ...
  return (
    <View className='flex-1 mx-6 '>
      ...
      <View className='mt-16'>
        <Text className='text-3xl font-bold'>What's your full name?</Text>
        <Text className='text-neutral-600 '>Add your name so friends can find you.</Text>
      </View>
